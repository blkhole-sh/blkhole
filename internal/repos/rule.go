...
func (rr *ruleRepo) Create(r *model.Rule) (int, error) {
	sql := "INSERT INTO rule (domain, allowed) VALUES (?, ?) RETURNING id"
	err := rr.db.QueryRowContext(rr.ctx, sql, r.Domain, r.Allowed).Scan(&r.ID)
	return r.ID, err
}

// Update modifies an existing rule with given ID in the database
func (rr *ruleRepo) Update(id int, r *model.Rule) error {
	sql := "UPDATE rule SET domain=?, allowed=? WHERE id=?"
	_, err := rr.db.ExecContext(rr.ctx, sql, r.Domain, r.Allowed, id)
	return err
}
...
func (rr *ruleRepo) FindByID(id int) (*model.Rule, error) {
	sql := "SELECT id, domain, allowed FROM rule WHERE id=?"
	var r model.Rule
...
func (rr *ruleRepo) FindByDomain(domain string) ([]*model.Rule, error) {
	sql := "SELECT id, domain, allowed FROM rule WHERE domain=?"
	var rules []*model.Rule
...
func (rr *ruleRepo) LinkToList(ruleID int, listID int) error {
	sql := "INSERT INTO list_rule (list_id, rule_id) VALUES (?, ?)"
	_, err := rr.db.ExecContext(rr.ctx, sql, listID, ruleID)
	return err
}

// UnlinkFromList removes a link between a rule and a list
func (rr *ruleRepo) UnlinkFromList(ruleID int, listID int) error {
	sql := "DELETE FROM list_rule WHERE list_id=? AND rule_id=?"
	_, err := rr.db.ExecContext(rr.ctx, sql, listID, ruleID)
	return err
}
