...
export async function getDevices(): Promise<Device[]> {
	...
	return (
		await fetch(`http://127.0.0.1:8080/users/${userHash}/devices`)
	).json();
}

export async function getLists(): Promise<List[]> {
	const userHash = useAuth();
	return (await fetch(`http://127.0.0.1:8080/users/${userHash}/lists`)).json();
}

export async function getSchedules(): Promise<Schedule[]> {
	const userHash = useAuth();
	return (
		await fetch(`http://127.0.0.1:8080/users/${userHash}/schedules`)
	).json();
}
